...
function ProjectModal({ open, onClose, onSubmit, initialData }) {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...initialData,
        secondary: (initialData.secondary || []).join(', '),
        consultant: initialData.consultant || ''
      });
      setPrevSecondary((initialData.secondary || []).join(', '));
    } else {
  ...
  const handleSubmit = () => {
    ...
    const invalidSecondary = secondaryArray.find(name =>
      name.toLowerCase() === primary.toLowerCase() ||
      name.toLowerCase() === teamLead.toLowerCase()
    );
    ...
    if (secondaryArray.length > 3) {
      setToast({
        open: true,
        message: 'Maximum 3 members allowed in Secondary.'
      });
      return;
    }

    setPrevSecondary(formData.secondary);
    onSubmit({ ...formData, secondary: secondaryArray });
  };
